...
class Config:
    ...
    # ══════════════════════════════════════════
    # 🚦 حماية من إغراق الطلبات (rate limiting) — حد أدنى بالثواني
    #    بين كل رسالة/أمر من نفس chat_id (لا يشمل ضغط الأزرار
    #    التي تُتابع طلباً بدأ للتو، فقط الرسائل الجديدة)
    # ══════════════════════════════════════════
    RATE_LIMIT_SECONDS: float = float(os.getenv("RATE_LIMIT_SECONDS", "3"))

    # ══════════════════════════════════════════
    # ✂️ حد أقصى لمدة "تنزيل جزء محدد بالوقت" (بالثواني) — يمنع طلب
    #    قص جزء طويل جداً يستهلك CPU/RAM/قرص بلا داعٍ
    # ══════════════════════════════════════════
    MAX_CLIP_DURATION_SECONDS: int = int(os.getenv("MAX_CLIP_DURATION_SECONDS", "600"))

    # ══════════════════════════════════════════
    # 🔐 معرّفات chat_id المخوَّلة لأوامر الإدارة (مثل /clear_cache)
    #    مفصولة بفواصل، مثال: ADMIN_CHAT_IDS=111111,222222
    # ══════════════════════════════════════════
    ADMIN_CHAT_IDS: list = [
