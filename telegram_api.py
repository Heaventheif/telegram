...
# ══════════════════════════════════════════════
# 🧹 تطهير اسم الملف المعروض عند الإرسال إلى تيليجرام
# ══════════════════════════════════════════════
_INVALID_FILENAME_CHARS = re.compile(r'[<>:"/\\|?*\x00-\x1f]')


def sanitize_filename(name: str, max_len: int = 150) -> str:
    """يطهّر اسم الملف قبل إرساله إلى تيليجرام (حقل filename في multipart).
    هذا الاسم لا يُستخدم كمسار فعلي على القرص (الملف نفسه دائماً مسار
    tempfile آمن تولّده الدالة المستدعية)، لكن التطهير طبقة حماية إضافية:
    يمنع اسم ملف يحتوي فواصل مسارات (../, \\) قد تُربك عملاء تيليجرام
    المختلفة، ويزيل محارف تحكّم/محارف غير صالحة في أنظمة الملفات الشائعة."""
    if not name:
        return "file"
    name = os.path.basename(name)  # يتجاهل أي مسار مضمَّن، يُبقي اسم الملف فقط
...
class Bot:
    ...
    async def _send_file(self, method: str, field: str, chat_id, file_path: str,
        ...
        fname = sanitize_filename(filename or os.path.basename(file_path))
